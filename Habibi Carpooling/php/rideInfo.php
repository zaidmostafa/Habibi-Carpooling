...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['removePassenger']) && $isPassenger) {
        ...
        // Update the passengers list in the database
        $sqlUpdate = "UPDATE rides SET passengersList = ?, passengersInt = ? WHERE rideID = ?";
        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bind_param("sii", $newPassengersList, $rideID);
        $stmtUpdate->execute();
        $stmtUpdate->close();

        // Redirect back to the ride info page after removal
        $conn->close();
        header("Location: rideInfo.php?rideID=" . $rideID);
        exit;
    }
}
?>
